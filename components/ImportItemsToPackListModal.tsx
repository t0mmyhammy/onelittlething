...
interface ImportItemsToPackListModalProps {
  packListId: string;
  children: Child[];
  onClose: () => void;
...
export default function ImportItemsToPackListModal({
  packListId,
  children,
  onClose,
  onImportComplete,
}: ImportItemsToPackListModalProps) {
  ...
  const handleImport = async () => {
    ...
    try {
      const response = await fetch('/api/import-pack-list-items', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          packListId,
        ...
        <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between rounded-t-2xl">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xl font-serif text-gray-900">
              {step === 'input' ? 'Import Items' : 'Review & Add Items'}
            </h2>
          </div>
          <button
            onClick={onClose}
          ...
          {step === 'input' ? (
            <>
              <p className="text-gray-600 text-sm mb-4">
                Paste items you want to add to this pack list. AI will organize them into categories.
              </p>

              <textarea
                value={inputText}
                onChange={(e) => setInputText(e.target.value)}
                placeholder="Example:&#10;&#10;Emma's rain jacket&#10;Lily's boots&#10;Snacks - granola bars, fruit snacks&#10;Chargers for phones&#10;&#10;Or paste a full list and let AI categorize it!"
                rows={14}
            ...
            <>
              <p className="text-gray-600 text-sm mb-4">
                {parsedCategories.length} {parsedCategories.length === 1 ? 'category' : 'categories'} found. Review and edit before adding to this pack list.
              </p>
              ...
              <div className="flex gap-3">
                ...
                >
                  {loading ? 'Adding...' : 'Add to Pack List'}
                </button>
              </div>
            </>
          )}
